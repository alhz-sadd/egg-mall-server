...
let lines = fs.readFileSync(routerPath, 'utf-8').split('\n');

// Find where "// 管理端公开接口" starts (around line 213)
const startIndex = lines.findIndex(line => line.includes('// 管理端公开接口'));

if (startIndex !== -1) {
  // Remove everything from startIndex to the end except the last line '};'
  lines = lines.slice(0, startIndex);
  lines.push('};');
}

let routerContent = lines.join('\n');
...
// Make sure to replace controller.admin.upload.image with controller.adminInner.upload.image
routerContent = routerContent.replace(
  /controller\.admin\.upload\.image/g,
  'controller.adminInner.upload.image',
);
