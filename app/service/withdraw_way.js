 ...
 * 处理管理端提现方式的增删改查
 */
class WithdrawWayService extends Service {
  /**
   * 格式化时间
  ...
  async adminList(query = {}, adminId) {
    ...
    const { count, rows } = await ctx.model.WithdrawWay.findAndCountAll({
      where,
  ...
  /**
   * 移动端获取启用的提现方式列表
   * 仅返回 status=0（启用）的提现方式，按 sort 升序、id 降序排列
   ...
   * @param adminId
   * @return {Object} 创建的提现方式
   */
  async adminCreate(payload, adminId) {
    const { ctx } = this;
    ctx.assert(payload.way, 422, '提现方式名称不能为空');
    ...
    const item = await ctx.model.WithdrawWay.create(createData);
  ...
  /**
   * 管理端更新提现方式
   * @param {number} id 提现方式ID
   * @param {Object} payload 请求参数
   * @return {Object} 更新后的提现方式
   */
  async adminUpdate(id, payload) {
    const { ctx } = this;
    const item = await ctx.model.WithdrawWay.findByPk(id);
    ctx.assert(item, 404, '提现方式不存在');
  ...
  async adminDestroy(id, adminId) {
    ...
    if (adminId !== undefined && item.admin_id !== adminId) {
      ctx.throw(403, '无权操作该店铺的提现方式');
    }
    await item.destroy();
  }
}

module.exports = WithdrawWayService;
